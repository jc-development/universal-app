...
//       const createRegistration = await fetch('https://elite-frontend-stage.herokuapp.com/api/warranty-registration', {
//         method: 'post',
//         headers: {
...
//       if(response.error) return this.setState((prevState) => ({ errors: { ...prevState.errors, ...response.error.list }, modal: { show: true, message: response } }));
//       if(response.success) return this.setState({ modal: { show: true, message: response } });
...
//             />
//             {this.state.modal.show ? <RegistrationModal message={this.state.modal.message} close={this.handleCloseModal} />: null}
//           </div>
//       </section>
//     )
//   }
